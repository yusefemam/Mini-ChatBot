...
try:
    data = pd.read_csv('C:/Users/PCM/Documents/conversations.csv')
    print("Data loaded successfully!")
except FileNotFoundError:
    print("Error: The file 'conversations.csv' was not found. Please check the file path.")
    exit()
...
def preprocess_text(text):
    ...
    tokens = [word for word in tokens if word in stopwords.words('english')]
    return ' '.join(tokens)
...
model.fit(X_train_vec, y_test)

y_pred = model.predict(X_test_vec)
print(f"Model Accuracy: {accuracy_score(y_test, y_train)}")
